...
function App() {
  ...
  const addHolding = holding => setPortfolio(p => { if (p.some(h => h.symbol === holding.symbol)) { setToast(`${holding.symbol} is already in your simulation`); return p; } setToast(`${holding.symbol} added to your simulation`); return [...p, { ...holding, income: holding.value * holding.yield / 100, color: holding.color || '#ef789a' }]; });
